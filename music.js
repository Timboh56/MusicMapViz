   ...
   lastfm.getTop = function(code,country,list,show){
	...
	$.ajax({
      ...
      success: function(r){
		...
		list == "artists" ? recv_list = r.topartists.artist : recv_list = r.toptracks.track;
		
		for (i in recv_list) {
			var text = "#text",
				c = parseInt(i,10) + 1,
				html_string,	
				listeners = recv_list[i].listeners;		
							
			if (list == "artists") {
				html_string = "<li> " + c + ". <img src='" + 
					recv_list[i].image[0][text] + "' > <a href='" + 
					recv_list[i].url + "'> " + 
					recv_list[i].name+"</a>, " + 
					recv_list[i].listeners + 
					" listeners \n </li>";
			} else {
				html_string = "<li> " + c +". " + 
					recv_list[i].name +" , " + 
					recv_list[i].artist.name + " , " +  
					recv_list[i].listeners + 
					" listeners \n </li>";
			}
		...
		show == true ? showForCountry(list_elems,list,country) : saveList(list_elems,code);
	  }
	});
   	...
   	success: function(response){ 
   		...
   		for ( var v in response.artists.artist ) {
				var name = response.artists.artist[v].name;
   				$("#topchart").append(" <li> <a href=\"#\" onclick=\"select('" + name + "');\" > " 
					+ name + "</a> </li> ");
   			}  		
   		}
   	 });
